...
- **Storage-format aware** – Gene-streaming functions (`wilcoxon_test`, `batch_process`) convert a CSR source once to a temporary, memory-bounded CSC copy beside the output (`format_mismatch_policy="convert"`, the default) instead of re-reading the whole matrix once per gene chunk; `convert_to_csc`/`convert_to_csr` honour `memory_limit_gb` and preserve the value dtype
